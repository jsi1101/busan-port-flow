...
function renderGates() { document.getElementById('gateCards').innerHTML = getGates().map((gate) => { const estimate = window.gateEstimates?.[gate.id]; const best = window.bestGateId === gate.id; return `<article class="app-gate-card" style="${best ? 'border:2px solid #1576d2;background:#eef7ff;' : ''}"><div><small>${best ? '★ 최적 경로' : ''}</small><h3>${gate.name}</h3><p>${estimate ? `이동 ${estimate.duration}분 + 예상 대기 ${estimate.wait}분` : '위치를 설정하면 총 소요시간을 계산합니다'}</p></div><strong>${estimate ? estimate.total : '-'}<em>${estimate ? '분' : ''}</em></strong></article>`; }).join(''); }
